...
public static class ExplosionScript
{

    public static int scrapLimit = 300;

    public static List<Disposable> scraps = new List<Disposable>();
    ...
    public static List<GameObject> GenerateTriangularPieces(GameObject source, int extraPoints = 0, int subshatterSteps = 0, Material mat = null)
    {
        ...
        if(rigidbody == null && source.transform.parent != null)
        {
            rigidbody = source.transform.parent.gameObject.GetComponent<Rigidbody2D>();
        }
        if(rigidbody != null)
        {
        ...
        if (subshatterSteps > 0)
        {
            ...
            {
                morePieces.AddRange(ExplosionScript.GenerateTriangularPieces(piece, extraPoints, subshatterSteps, mat));
    ...
    private static GameObject generateTriangularPiece(GameObject source, List<Vector2> tri, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat)
    {
        ...
        Vector2 forcevector = new Vector2((Random.value * 2 - 1) * 100f, (Random.value * 2 - 1) * 100f);
     //   Debug.Log(forcevector.ToString());
        rigidbody.AddForce(forcevector);

        var dis = piece.AddComponent<Disposable>();
        dis.deathTime = source.GetComponent<Destroyable>().fuse;
        dis.fadeoutTime = source.GetComponent<Destroyable>().fadeOut;

        scraps.Add(dis);

        if(scraps.Count > scrapLimit)
        {
            scraps[0].Delete();
        }
    ...
    public static List<GameObject> GenerateVoronoiPieces(GameObject source, int extraPoints = 0, int subshatterSteps = 0, Material mat = null)
    {
        ...
        if (subshatterSteps > 0)
        {
            ...
            foreach (GameObject piece in pieces)
            {
                morePieces.AddRange(ExplosionScript.GenerateVoronoiPieces(piece, extraPoints, subshatterSteps));
                GameObject.DestroyImmediate(piece);
            }
        }
        else
